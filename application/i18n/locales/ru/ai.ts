...
export const ruAiMessages: Messages = {
  ...
  'ai.codebuddy.persistSession.hint': 'Сохраняет историю диалога на диск, чтобы сессию можно было возобновить позже. Отключите, чтобы держать её только в памяти — ничего не записывается, а контрольные точки файлов пропускаются. Требуется CodeBuddy CLI 2.125.1 или новее.',
